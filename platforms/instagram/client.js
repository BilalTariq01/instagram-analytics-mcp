...
export class InstagramClient {
    ...
    async getAccountId() {
        ...
        try {
            ...
            const accountList = accounts.map((acc, idx) => `${idx + 1}. @${acc.username} (${acc.name}) - ID: ${acc.id}`).join('\n');
            throw new Error(`Multiple Instagram accounts found. Please specify which account to use by adding the account_id parameter:\n\n${accountList}\n\nExample:\n{\n  "account_id": "${accounts[0].id}"\n}\n\nOr use list_available_accounts to see all accounts.`);
        }
        catch (error) {
            if (axios.isAxiosError(error)) {
     ...
     */
    async getAccountInsights(metrics, period, options) {
        try {
            const targetAccountId = options?.accountId || await this.getAccountId();
            // Check if demographic metrics are requested
            ...
            // If demographic metrics are requested but no breakdown is provided, use 'country' as default
            ...
            // Add breakdown if specified or required for demographic metrics
            if (breakdown) {
            ...
            const response = await this.axios.get(`/${targetAccountId}/insights`, { params });
     ...
     * Get media list
     */
    async getMedia(limit = 25, accountId) {
        try {
            const targetAccountId = accountId || await this.getAccountId();
            const response = await this.axios.get(`/${targetAccountId}/media`, {
                params: {
     ...
     */
    async getMediaInsights(mediaId, metrics, period = 'lifetime') {
        try {
            const response = await this.axios.get(`/${mediaId}/insights`, {
                params: {
                    metric: metrics.join(','),
                    period,
                },
